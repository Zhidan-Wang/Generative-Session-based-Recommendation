...
class NARM(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        item_seq_emb = self.item_embedding(item_seq) #[1, seq_len, emb]
        item_seq_emb_dropout = self.emb_dropout(item_seq_emb)
        gru_out, _ = self.gru(item_seq_emb_dropout) #[1, seq_len, hidden]

        # fetch the last hidden state of last timestamp
        c_global = ht = self.gather_indexes(gru_out, item_seq_len - 1) #[1, hidden]
        # avoid the influence of padding
        mask = item_seq.gt(0).unsqueeze(2).expand_as(gru_out) # 1, seq_len, hidden (0/1))
        q1 = self.a_1(gru_out)
        q2 = self.a_2(ht)
        q2_expand = q2.unsqueeze(1).expand_as(q1)
        # calculate weighted factors α
        alpha = self.v_t(mask * torch.sigmoid(q1 + q2_expand)) # 1, seq_len, hidden
        c_local = torch.sum(alpha.expand_as(gru_out) * gru_out, 1) # 1,  hidden
        c_t = torch.cat([c_local, c_global], 1)
        c_t = self.ct_dropout(c_t)
        seq_output = self.b(c_t)
        return seq_output # 1, emb

    def calculate_loss(self, item_seq, item_seq_len, pos_items):
        seq_output = self.forward(item_seq, item_seq_len)
        
        test_item_emb = self.item_embedding.weight
        logits = torch.matmul(seq_output, test_item_emb.transpose(0, 1)) #[batch, n_items]
